...
def get_token(user):
    payload = {
        ...
        "mobile":user.mobile,
        "email":user.email,
        "exp": datetime.datetime.utcnow() + datetime.timedelta(hours=12),  # Token expiry (1 hour)
...
def token_validation(request):
    if request.method == "POST":
        ...
        try:
            ...
            return JsonResponse({"message":"Token Verified"},status=200)  # Returns user data from token
       ...
       # Step 1: Filter by State
        ...
        for land in land_records:
            ...
            response_data.append({
                ...
                "images": image_base64_list  # Sending images as Base64
            })
